...
from removalmodels.models import GeneratorDiff, GeneratorDiffWithInp, GeneratorDiffAndMask, GeneratorDiffAndMask_V2, \
    GeneratorBoxReconst, GeneratorOnlyMask, GeneratorMaskAndFeat, GeneratorMaskAndFeat_ImNetBackbone, \
    GeneratorMaskAndFeat_ImNetBackbone_V2
from removalmodels.models import Discriminator_SN, Discriminator, DiscriminatorBBOX, DiscriminatorGlobalLocal, \
    DiscriminatorGlobalLocal_SN, DiscriminatorGAP, DiscriminatorGAP_ImageNet
...
class Solver(object):
    ...
    def build_model(self, pretrainedcv):
        ...
        if (self.mode == 'train' or self.mode == 'eval'):
            if (self.train_boxreconst == 1 or self.train_boxreconst == 2):
                ...
                self.D = DisClass(self.image_size, self.d_conv_dim, self.c_dim, self.d_repeat_num,
                                  self.d_init_stride, max_filters=self.d_max_filters,
                                  nc=self.nc * (1 + self.discrim_masked_image),
                                  d_kernel_size=self.d_kernel_size, patch_size=self.d_patch_size,
                                  use_tv_inp=self.use_tv_inp)
                if self.use_seperate_classifier and not self.train_only_g:
                    if self.use_imagenet_pretrained is not None:
                        self.D_cls = DiscriminatorGAP_ImageNet(self.image_size, self.c_dim,
                                                               net_type=self.use_imagenet_pretrained,
                                                               max_filters=self.d_max_filters_cls,
                                                               global_pool=self.d_global_pool)
                    else:
                        DisClass = DiscriminatorGAP if self.use_gap_classifier else DisClass
                        self.D_cls = DisClass(self.image_size, self.d_conv_dim, self.c_dim,
                                              4 if self.use_gap_classifier else self.d_repeat_num,
                                              self.d_init_stride, max_filters=self.d_max_filters_cls,
        ...
        if not self.train_only_g:
            if ((self.train_boxreconst == 1) and not self.full_image_encoder) and (self.boxfeat_dim > 0):
                ...
                self.E = maskgen(self.g_conv_dim, self.c_dim, self.e_repeat_num, 0, binary_mask=self.g_binary_mask,
                                 out_feat_dim=self.boxfeat_dim,
                                 mask_size=self.mask_size if self.image_size == 128 else self.mask_size // 2,
                                 additional_cond=self.mask_additional_cond, per_classMask=self.perclass_mask,
                                 noInpLabel=self.mask_noinplabel, mask_normalize=self.mask_normalize_byclass,
                                 nc=4 if self.use_gtmask_inp else 3,
        ...
        if self.use_maskprior_gan:
            ...
            self.mask_D = Discriminator(self.mask_size, self.d_conv_dim, self.c_dim, int(np.log2(self.mask_size) - 1),
                                        self.d_init_stride, classify_branch=classify_type,
                                        max_filters=256 if self.c_dim == 1 else 512,
                                        nc=1)
        ...
        if (self.D_cls is not None) and self.mode == 'train' and (
        not self.d_use_spectralnorm) and self.use_seperate_classifier and (self.use_imagenet_pretrained is None):
        ...
        if self.mode == 'train':
            if self.g_upsample_type == 'deform':
                ...
                g_params.append({'params': [p[1] for p in self.G.named_parameters() if 'coordfilter' in p[0]],
                                 'lr': self.g_lr * 0.1})
            else:
                g_params = self.G.parameters()

            self.g_optimizer = torch.optim.Adam(g_params, self.g_lr, [self.beta1, self.beta2])  # , amsgrad=True)
            self.d_optimizer = torch.optim.Adam([p for p in self.D.parameters() if p.requires_grad], self.d_lr,
                                                [self.beta1, self.beta2])  # , amsgrad=True)
            if (self.D_cls is not None) and self.use_seperate_classifier:
                self.dcls_optimizer = torch.optim.Adam([p for p in self.D_cls.parameters() if p.requires_grad],
                                                       self.d_lr, [self.beta1, self.beta2],
                                                       weight_decay=0.01)  # , amsgrad=True)
            if self.E is not None:
                self.e_optimizer = torch.optim.Adam([p for p in self.E.parameters() if p.requires_grad], self.e_lr,
                                                    [self.beta1, self.beta2])  # , amsgrad=True)
            if self.use_maskprior_gan:
                self.mask_d_optimizer = torch.optim.Adam([p for p in self.mask_D.parameters() if p.requires_grad],
                                                         self.d_lr, [self.beta1, self.beta2])  # , amsgrad=True)
        ...
        if torch.cuda.is_available():
            ...
            if (self.mode == 'train' or self.mode == 'eval'):
                self.D.cuda()
            if (self.D_cls is not None) and self.use_seperate_classifier and (
                    self.mode == 'train' or self.mode == 'eval'):
                self.D_cls.cuda()
            if self.E is not None:
                self.E.cuda()
    ...
    def getImageSizeMask(self, mask):
        return F.upsample(mask, scale_factor=int(self.image_size / mask.size(-1)),
                          mode=self.m_upsample_type) if mask.size(-1) != self.image_size else mask
    ...
    def load_pretrained_model(self, cv=None):
        ...
        if (
                self.mode == 'train' or self.mode == 'eval') and self.load_discriminator:  # and not self.d_use_spectralnorm:
    ...
    def forward_generator(self, real_x, boxImg=None, imagelabel=None, mask=None, boxlabel=None, bbox=None,
                          get_feat=None, onlyMasks=False, mask_threshold=0.3, withGTMask=False, dilate=None, n_iter=0):

        if self.train_boxreconst == 3:
            fake_x, _, mask = self.forward_fulleditor(real_x, imagelabel, onlyMasks=onlyMasks,
                                                      mask_threshold=mask_threshold, gtMask=mask, withGTMask=withGTMask,
                                                      dilate=dilate, n_iter=n_iter)
        ...
        for e in range(start, self.num_epochs):
            for i, (real_x, real_label) in enumerate(self.data_loader):
                ...
                if self.dataset == 'coco':
                    ...
                    # XXX: FIX THIS!!!!!
                    fake_label[aidx, rand_idx] = 1. - real_label[aidx, rand_idx] if not self.arch[
                        'only_remove_train'] else 0.
                else:
                ...
                if (i + 1) % self.log_step == 0:
                    ...
                    accLog['acc'] = accLog['acc'] * 0.8 + (
                                1 - 0.8) * accuracies.data.cpu().numpy() if 'acc' in accLog else accuracies.data.cpu().numpy()
                    log = ["{}: {:.2f}".format(self.selected_attrs[cati], acc) for cati, acc in
                           enumerate(accLog['acc'])]
                    print(log)
                ...
                if (i + 1) % self.d_train_repeat == 0:
                    ...
                    if self.lambda_rec:
                        ...
                        if self.adv_rec:
                            g_loss_rec_fake = - torch.mean(out_src_rec) if self.use_topk_patch == 0 else - \
                            out_src_rec.view(cbsz, -1).topk(self.use_topk_patch, dim=1, largest=False)[0].mean()
                ...
                if (i + 1) % self.model_save_step == 0:
                    ...
                    save_checkpoint(checkpointData,
                                    fappend=self.arch['fappend'] + '_{}_{}.pth.tar'.format(e + 1, i + 1),
    ...
    def train_fulleditor(self):
        ...
        for e in range(start, self.num_epochs):
            ...
            train_m = (not self.fixed_m) if self.alternate_mask_train < 2 else (not self.fixed_m) if (
                        ((e % self.train_g_every) != 1) and self.alternate_mask_train == 2) else False
            train_g = self.g_fine_tune if (self.alternate_mask_train < 2) or self.fixed_m else self.g_fine_tune if (
                        ((e % self.train_g_every) == 1) and self.alternate_mask_train == 2) else 0
            ...
            for i, (real_x, real_label, boxImg, boxlabel, randmask, bbox, curCls) in enumerate(self.data_loader):
                ...
                # XXX: FIX THIS!!!!!
                fake_label[
                    aidx, rand_idx] = 0.  # 1. - real_label[aidx,rand_idx] if not self.arch['only_remove_train'] else 0.
                ...
                if self.only_random_boxes_discr:
                    ...
                    if train_g and not self.train_g_wo_m:
                        fake_x_full, _, f_mask = self.forward_fulleditor(real_x, mask_target, binary_mask=False,
                                                                         gtMask=gtMask)
                        fake_x_full.detach()
                else:
                    fake_x, _, f_mask = self.forward_fulleditor(real_x, mask_target, binary_mask=False, gtMask=gtMask,
                                                                dilate=dilateWeight)
                    fake_x = fake_x.detach()
                ...
                if train_g:
                    ...
                    dInp_real_x = real_x if not self.discrim_masked_image else torch.cat(
                        [real_x, real_x * (randmask if rn else self.getImageSizeMask(f_mask))], dim=1)
                    out_src, out_cls_real = self.D(dInp_real_x)
                    if self.d_local_supervision:
                        d_loss_real_p, d_loss_real_n, n_dlr_p, n_dlr_n = self.compute_real_fake_loss_local(out_src,
                                                                                                           self.adv_loss_type,
                                                                                                           F.adaptive_max_pool2d(
                                                                                                               (
                                                                                                                   randmask if rn else self.getImageSizeMask(
                                                                                                                       f_mask)),
                                                                                                               self.d_patch_size),
                                                                                                           only_pos=True)
                    else:
                    ...
                    dInp_fake_x = fake_x if not self.discrim_masked_image else torch.cat([fake_x, fake_x * (
                        randmask if self.only_random_boxes_discr else self.getImageSizeMask(f_mask))], dim=1)
                    out_src_fake, out_cls_fake = self.D(dInp_fake_x)
                    if self.d_local_supervision:
                        d_loss_fake_p, d_loss_fake_n, n_dlf_p, n_dlf_n = self.compute_real_fake_loss_local(out_src_fake,
                                                                                                           self.adv_loss_type,
                                                                                                           F.adaptive_max_pool2d(
                                                                                                               randmask if self.only_random_boxes_discr else f_mask,
                                                                                                               self.d_patch_size))
                    else:
                    ...
                    if not self.only_random_boxes_discr:
                        ...
                        if self.train_g_wo_m:
                            ...
                            dInp_fake_x_full = fake_x_full if not self.discrim_masked_image else torch.cat(
                                [fake_x_full, fake_x_full * self.getImageSizeMask(f_mask)], dim=1)
                            out_src_fake_full, _ = self.D(dInp_fake_x_full)
                            if self.d_local_supervision:
                                d_loss_fake_full_p, d_loss_fake_full_n, n_dlff_p, n_dlff_n = self.compute_real_fake_loss_local(
                                    out_src_fake_full, self.adv_loss_type,
                                    F.adaptive_max_pool2d(f_mask, self.d_patch_size))
                            else:
                                d_loss_fake_full = self.compute_real_fake_loss(out_src_fake_full, self.adv_loss_type,
                                                                               datasrc='fake')
                            if not self.d_local_supervision:
                                d_loss = 2.0 * d_loss_real + d_loss_fake + d_loss_fake_full
                            else:
                                d_loss_real = (d_loss_real_p + d_loss_fake_p + d_loss_fake_full_p) / (
                                            1e-8 + n_dlr_p + n_dlf_p + n_dlff_p)
                                d_loss_fake = (d_loss_real_n + d_loss_fake_n + d_loss_fake_full_n) / (
                                            1e-8 + n_dlr_n + n_dlf_n + n_dlff_n)
                                d_loss = d_loss_real + d_loss_fake
                ...
                if train_m and (not self.fixed_classifier):
                    ...
                    d_loss_cls = F.binary_cross_entropy_with_logits(
                        out_cls_real.view(cbsz, -1), np.random.uniform(0.95, 1.0) * real_label,
                        size_average=False) / cbsz
                    ...
                    if self.adv_classifier:
                        ...
                        if len(nonFlipped_idx):
                            ...
                            d_loss_cls = d_loss_cls + factor * (F.binary_cross_entropy_with_logits(
                                out_cls_fake[nonFlipped_idx, :],
                                np.random.uniform(0.95, 1.0) * real_label[nonFlipped_idx, :], size_average=False) / len(
                                nonFlipped_idx))
                    elif self.train_robust_d:
                        factor = 1.0
                        d_loss_cls = d_loss_cls + factor * (
                                    F.binary_cross_entropy_with_logits(out_cls_fake.view(cbsz, -1), fake_label,
                                                                       size_average=False) / cbsz)
                    ...
                    if (i + 1) % self.log_step == 0:
                        ...
                        accLog['r_acc'] = accLog['r_acc'] * 0.8 + (
                                    1 - 0.8) * r_accuracies.data.cpu().numpy() if 'r_acc' in accLog else r_accuracies.data.cpu().numpy()
                        if (not self.only_random_boxes_discr) and (self.adv_classifier or self.train_robust_d):
                            ...
                            accLog['f_acc'] = accLog['f_acc'] * 0.8 + (
                                        1 - 0.8) * f_accuracies.data.cpu().numpy() if 'f_acc' in accLog else f_accuracies.data.cpu().numpy()
                ...
                # ------------------------------------------------
                if self.use_maskprior_gan and len(nnz_idx) > 0 and (not self.fixed_m) and train_m and (
                not self.train_only_d):
                    if self.only_random_boxes_discr:
                        _, _, f_mask = self.forward_fulleditor(real_x, mask_target, binary_mask=False, onlyMasks=True,
                                                               gtMask=gtMask)
                        f_mask.detach()
                    ...
                    if not (self.d_use_spectralnorm):
                        mask_d_loss_gp = self.lambda_gp * self.compute_gradient_penalty(real_mask, f_mask,
                                                                                        discrim_type='mask', label=(
                                                                                                                               real_mlabel + fm_targ) / 2.)
                        self.reset_grad()
                ...
                if (i + 1) % self.d_train_repeat == 0 and (not self.train_only_d):
                    for p in self.G.parameters():
                        p.requires_grad = p.requires_grad_orig if (self.alternate_mask_train == 2 and train_g) or (
                            not self.alternate_mask_train) else False  #
                    ...
                    if self.train_g_wo_m:
                        ...
                        fake_x, _, mask, allMasks = self.forward_fulleditor(real_x, mask_target,
                                                                            binary_mask=not train_m, gtMask=gtMask,
                                                                            getAllMasks=True)
                        if train_g:
                    ...
                    if train_m:
                        ...
                        if self.lambda_maskL1loss and not self.grad_weighted_l1loss:
                            hingeLoss = torch.clamp(mask - 0.1, min=0.).mean(dim=2).mean(
                    ...
                    if len(fake_x):
                        ...
                        dInp_fake_x = fake_x if not self.discrim_masked_image else torch.cat(
                            [fake_x, fake_x * self.getImageSizeMask(mask)], dim=1)
                    ...
                    if train_m:
                        if self.useclsweights:
                            cls_scale = self.clsweight_scale_init ** (
                                        e - start) if self.clsweight_scale_decay else self.clsweight_scale_init
                            weights = F.sigmoid(
                                cls_scale * out_cls_real.detach().view(cbsz, -1)[nnz_idx, rand_idx[nnz_idx]])
                        else:
                            weights = None
                        if self.onlytargetclsloss == 1:
                            if len(nnz_idx) > 0:
                                g_loss_cls = F.binary_cross_entropy_with_logits(
                                    out_cls.view(cbsz, -1)[nnz_idx, rand_idx[nnz_idx]],
                                    fake_label[nnz_idx, rand_idx[nnz_idx]], size_average=False, weight=weights) / len(
                                    nnz_idx)
                            else:
                                g_loss_cls = 0.
                            ...
                            if len(nnz_idx):
                                ...
                                g_loss_cls = 1.2 * F.binary_cross_entropy_with_logits(scores_remove, torch.zeros_like(
                                    scores_remove).detach(), weight=weights)
                                ...
                                if self.useclsweights:
                                    weights = F.sigmoid(
                                        cls_scale * scores_retain_real[(scores_retain_real > scores_retain).detach()])
                                else:
                                    weights = None

                                # g_loss_cls += torch.pow(torch.clamp(torch.clamp(scores_retain_real,max=3.) - scores_retain,min=0.),1.5).mean()
                                g_loss_cls += F.binary_cross_entropy_with_logits(scores_retain_damaged, torch.ones_like(
                                    scores_retain_damaged).detach(), weight=weights)
                        else:
                            g_loss_cls = F.binary_cross_entropy_with_logits(out_cls.view(cbsz, -1), fake_label,
                                                                            size_average=False) / cbsz
                    ...
                    if len(fake_x) and train_g:
                        if self.d_local_supervision:
                            g_loss_fake, _, n_glf_p, _ = self.compute_real_fake_loss_local(out_src, self.adv_loss_type,
                                                                                           F.adaptive_max_pool2d(mask,
                                                                                                                 self.d_patch_size),
                                                                                           only_pos=True)
                    ...
                    if train_m and train_g:
                        ...
                        if self.lambda_maskL1loss and self.grad_weighted_l1loss:
                            ...
                            hingeLoss = torch.clamp(mask * grad_weight - 0.1, min=0.).mean(dim=2).mean(
                                dim=2)  # (mask>0.1).float().mean(dim=2).mean(dim=2)
                            m_l1_loss = torch.exp(hingeLoss).mean()
                            loss['M/l_l1'] = loss['M/l_l1'] * avgexp + (1 - avgexp) * (
                                        self.lambda_maskL1loss * m_l1_loss.data[0]) if 'M/l_sm' in loss else (
                        ...
                        if self.use_random_boxes:
                            ...
                            if self.d_local_supervision:
                                g_loss_fake, _, n_glf_p, _ = self.compute_real_fake_loss_local(out_src,
                                                                                               self.adv_loss_type,
                                                                                               F.adaptive_max_pool2d(
                                                                                                   randmask,
                                                                                                   self.d_patch_size),
                                ...
                                g_loss_fake = self.compute_real_fake_loss(out_src, self.adv_loss_type,
                                                                          loss_for='generator')
                            self.reset_grad()
                            # Backward + Optimize
            ...
            if (e + 1) % print_iter == 0:
                if 'r_acc' in accLog:
                    log = ['Real--'] + ["{}: {:.2f}".format(self.selected_attrs[cati], acc) for cati, acc in
                                        enumerate(accLog['r_acc'])]
                    print((' '.join(log)))
                if 'f_acc' in accLog:
                    log = ['Fake--']
                    log.extend(["{}: {:.2f}".format(self.selected_attrs[cati], acc) for cati, acc in
                                enumerate(accLog['f_acc'])])
                    print((' '.join(log)))

            # Translate fixed images for debugging
            if ((e + 1) % self.sample_step == 0) and (not self.train_only_d):
                fake_image_list = [fixed_x]
                fake_x, _, mask = self.forward_fulleditor(fixed_x, fixed_mask_target, binary_mask=True,
                                                          gtMask=fixed_gTMask)
    ...
    def forward_boxreconst(self, x, box, mask, boxlabel=None, bbox=None, get_feat=False, no_enc=False):
        ...
        if self.E is not None and (not no_enc):
            if self.train_boxreconst == 2 or self.full_image_encoder:
                boxFeat = self.E(xInp if self.e_masked_image == 1 else torch.cat([x, xInp],
                                                                                 dim=1) if self.e_masked_image == 2 else torch.cat(
                    [x, mask], dim=1), boxlabel if self.use_box_label else None)
            else:
        ...
        if self.g_fixed_size:
            ...
            for i in range(bsz):
                ...
                fakeImg[i, :, bbox[i, 1]:bbox[i, 1] + bbox[i, 3],
                bbox[i, 0]:bbox[i, 0] + bbox[i, 2]] = F.adaptive_avg_pool2d(genImg[i:i + 1, :, :, :],
                                                                            (bbox[i, 3], bbox[i, 2]))
    ...
    def getRandBoxWith(self, img, boxestoInc=None):
        ...
        if boxestoInc is None:
            ...
            randomBoxes = [(random.randint(
                min(max(min(boxestoInc[i, 1] + boxestoInc[i, 3] - size, boxestoInc[i, 1]), 0), imsz - size),
                min(max(max(boxestoInc[i, 1] + boxestoInc[i, 3] - size, boxestoInc[i, 1]), 0), imsz - size)),
                            random.randint(
                                min(max(min(boxestoInc[i, 0] + boxestoInc[i, 2] - size, boxestoInc[i, 0]), 0),
                                    imsz - size),
                                min(max(max(boxestoInc[i, 0] + boxestoInc[i, 2] - size, boxestoInc[i, 0]), 0),
                                    imsz - size))) for i in range(bsz)]

        croppedImg = torch.cat(
            [img[i:i + 1, :, rb[0]:rb[0] + size, rb[1]:rb[1] + size] for i, rb in enumerate(randomBoxes)], dim=0)
    ...
    def train_reconst_nw(self):
        ...
        for e in range(start, self.num_epochs):
            for i, (real_x, real_label, boxImg, boxlabel, mask, bbox) in enumerate(self.data_loader):
                ...
                # XXX: FIX THIS!!!!!
                fake_boxlabel[aidx, rand_idx] = (1. - boxlabel[aidx, rand_idx]) if not self.arch[
                    'only_remove_train'] else 0.
                if self.use_box_label == 2:
                ...
                if not self.only_reconst_loss:
                    # Compute loss with real images
                    out_src, out_cls, _ = self.D(real_x, self.getRandBoxWith(real_x, boxestoInc=bbox),
                                                 classify=train_edit_mode)
                    ...
                    out_src_fake, out_cls_fake, _ = self.D(fake_x, self.getRandBoxWith(fake_x, boxestoInc=bbox),
                                                           classify=train_edit_mode)
                ...
                if ((((i + 1) % self.d_train_repeat) == 0)) or self.only_reconst_loss:
                    ...
                    fake_x = self.forward_boxreconst(real_x, boxImg, mask,
                                                     fake_boxlabel_inp if train_edit_mode else boxlabel, bbox)
                    ...
                    # Compute losses
                    if not self.only_reconst_loss:
                        out_src, out_cls, out_feat_fake = self.D(fake_x, self.getRandBoxWith(fake_x, bbox),
                                                                 classify=train_edit_mode)
                        g_loss_fake = self.compute_real_fake_loss(out_src, self.adv_loss_type, loss_for='generator')
                    else:
                        g_loss_fake = 0.
                    ...
                    if self.lambda_rec:
                        ...
                        if train_edit_mode:
                            ...
                            if self.adv_rec:
                                g_loss_rec = g_loss_rec - torch.mean(
                                    out_src_rec) + 2. * F.binary_cross_entropy_with_logits(out_cls_rec.view(cbsz, -1),
                                                                                           boxlabel,
                                                                                           size_average=False) / cbsz
                        else:
                    ...
                    if self.lambda_feat_match:
                        ...
                        if train_edit_mode:
                            ...
                            g_loss_feat = F.mse_loss(out_feat_fake.mean(dim=0),
                                                     out_feat_real[z_idx, ::].detach().mean(dim=0))
                        else:
            ...
            if (e + 1) % self.sample_step == 0:
                ...
                fake_image_list.append(
                    self.forward_boxreconst(fixed_x, fixed_box, fixed_mask, fixed_boxlabel, fixed_bbox))
                fake_images = torch.cat(fake_image_list, dim=3)
                save_image(self.denorm(fake_images.data),
                           os.path.join(self.sample_path, '{}_{}_fake.png'.format(e + 1, i + 1)), nrow=1, padding=0)
                print(('Translated images and saved into {}..!'.format(self.sample_path)))
    ...
    def train_multi(self):
        ...
        for i in range(start, self.num_iters):
            ...
            if (i + 1) % self.d_train_repeat == 0:
                ...
                g_loss_cls = F.binary_cross_entropy_with_logits(out_cls1, fake_label1,
                                                                size_average=False) / fake_x1.size(0)
